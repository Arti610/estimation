...
const EditProfile = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (userDetails.id) {
      ...
      try {
        setLoading(true)
        const response = await api.put(`/updateuser/${userDetails.id}`, fData, {
          headers: {
        ...
        if (response.statusText === "Created" || response.status === "201" || response.statusText === "OK" || response.status === "200") {
